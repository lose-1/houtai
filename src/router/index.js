...
const staticRoutes = [
	...
	},
	{
		path: '/ai',
		name: 'ai',
		component: () => import('/src/views/ai.vue'),
	},

	{path: '/knowledgeku/tinymceEditor/:id',name: 'tinymceEditor',component: () => import('@/views/knowledgeku/tinymceEditor.vue'),meta: {title: 'message.staticRoutes.tinymceEditor',},},
...
export function adminsUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuAdmins()
		.then(async (res) => {
...
// 添加动态路由，`{ path: '*', redirect: '/404' }` 防止页面刷新，静态路由丢失问题
export function zhuguanUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuZhuguan()
		.then(async (res) => {
...
export function usersUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuUsers()
		.then(async (res) => {
...
export function getRouterList(router, to, next) {
	...
	if (Session.get('userInfo').roles[0] === 'admins') adminsUser(router, to, next);
	if (Session.get('userInfo').roles[0] === 'users') usersUser(router, to, next);
	if (Session.get('userInfo').roles[0] === 'zhuguan') zhuguanUser(router, to, next);
