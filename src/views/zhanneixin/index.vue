<template>
	<div>
		<el-form :inline="true" :model="formInline" class="user-search">
			...
			<el-form-item label="">
				<el-select size="small" v-model="formInline.fasongjuese" placeholder="发送角色" clearable>
					<el-option label="管理员" value="admins"></el-option>
			...
			<el-form-item>
				...
				<el-button size="small" type="primary" icon="el-icon-plus" @click="handleEdit()" >发布新最邮件</el-button>
		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column  align="center" sortable prop="issh" label="对方是否已读" >
				<template slot-scope="scope" >
		...
		<el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
			<el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
				...
				<el-form-item  label="发件人" prop="fajianren"><el-input size="small" v-model="editForm.fajianren" auto-complete="off" placeholder="请输入发件人" style='width:50%' disabled></el-input></el-form-item>
				<el-form-item label="发送角色" prop="fasongjuese">
					<el-select v-model="editForm.fasongjuese" placeholder="请选择发送角色" style='width:50%'>
						<el-option label="管理员" value="admins" v-if="userInfo.roles.includes('admins')"></el-option>
						<el-option label="用户" value="users" v-if="userInfo.roles.includes('users')"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="接收角色" prop="jieshoujuese">
					<el-select v-model="editForm.jieshoujuese" placeholder="请选择接收角色" style='width:50%'>
						<el-option label="管理员" value="admins"></el-option>
						<el-option label="用户" value="users"></el-option>
					</el-select>
				</el-form-item>
...
export default {
	...
	data() {
		return {
			...
			rules: {
				...
				jieshoujuese: [{ required: true, message: '请选择接收角色', trigger: 'change' },
				],
				fasongjuese: [{ required: true, message: '请选择发送角色', trigger: 'change' },
				],
			},
	...
	created() {
		...
		this.formInline.fajianren= this.username;
	...
	methods: {
		...
		getdata(parameter) {
			// 如果不是管理员，添加角色和收件人过滤条件
			if (this.userInfo.roles[0] !== 'admins') {
				parameter.fasongjuese = this.userInfo.roles[0]; // 添加发送角色匹配条件
			}
		...
		handleEdit: function (index, row) {
			...
			if (row != undefined && row != 'undefined') {
				...
				this.title = '添加在线留言'
				this.editForm = {
					fajianren: this.username,
					jieshoujuese: this.userInfo.roles[0], // 默认设置为当前用户角色
					fasongjuese: this.userInfo.roles[0]  // 默认设置为当前用户角色
				}
