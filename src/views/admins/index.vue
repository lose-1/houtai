	...
	<div>
		<el-form :inline="true" :model="formInline" class="user-search">
			...
			<el-form-item>
				...
				<el-button size="small" type="primary" icon="el-icon-plus" @click="handleEdit()" v-if="userInfo.roles[0] == 'admins'">添加</el-button>
...
<!--					<el-button size="mini" type="success" @click="handleEdit(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'admins'">编辑</el-button>-->
					<el-button size="mini" type="danger" @click="deleteAdmins(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'admins'">删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-form-item label="密码" prop="pwd"><el-input size="small" v-model="editForm.pwd" auto-complete="off" placeholder="请输入密码" style='width:50%'></el-input></el-form-item>
...
import { adminsList, adminsSave, adminsDelete,adminsEdit,adminsDeleteList } from '@/api/admins/adminsApi';
	...
	methods: {
		...
		submitForm(editData) {
			this.$refs[editData].validate(valid => {
				if (valid) {
					if(this.editForm.id){
						adminsEdit(this.editForm).then(res => {
						...
						adminsSave(this.editForm).then(res => {
							this.editFormVisible = false
							this.loading = false
							if (res.code == '0') {
		...
		// 删除公司
		deleteAdmins(index, row) {
			this.$confirm('确定要删除吗?', '信息', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				adminsDelete(row.id).then(res => {
					if (res.code == '0') {
						this.$message({
								type: 'success',
							message: '管理员已删除!'
		...
		handleDeleteList(){
			...
			if(adminss.length == 0){
				...
				this.$confirm('确定要批量删除吗?', '信息', {
					...
					adminsDeleteList(adminss).then(res => {
						if (res.code == '0') {
							this.$message({
								type: 'success',
								message: '批量删除成功!'
