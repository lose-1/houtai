...
<script>
	...
	import {usersinfosUpdatePassword} from "@/api/usersinfos/usersinfosApi";
	
	import {adminsUpdatePassword} from "@/api/admins/adminsApi";
	export default {
		name: 'user',
		data() {
			return {
		...
		methods: {
			...
			submitForm: function (editData) {
					...
					if (valid) {
						if (this.editForm.newPassword === this.editForm.newPassword2) {
							...
							if (this.userInfo.roles[0] == 'admins') {
								adminsUpdatePassword(this.editForm).then(res => {
									if (res.code == "0") {
							...
							else if (this.userInfo.roles[0] == 'users') {
								usersinfosUpdatePassword(this.editForm).then(res => {
									if (res.code == "0") {
										this.$message.success('修改成功，为了安全，系统将自动清空session，请您重新登陆')
										setTimeout(() => {
											Session.clear();
              ...
              else if (this.userInfo.roles[0] == 'zhuguan') {
                zhuguanUpdatePassword(this.editForm).then(res => {
                  if (res.code == "0") {
                    this.$message.success('修改成功，为了安全，系统将自动清空session，请您重新登陆')
                    setTimeout(() => {
                      Session.clear();
