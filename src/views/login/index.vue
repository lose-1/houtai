        ...
        <el-form class="login-form" :model="ruleForm" :rules="rules" ref="ruleForm">
          ...
          <el-form-item prop="password">
            <el-input
                ...
                :show-password="true"
            >
              <i slot="suffix" class="el-icon-arrow-down password-icon"></i>
            </el-input>
          </el-form-item>
          ...
          <el-form-item prop="code" class="code-form-item">
            <div class="code-input-wrapper">
              <el-input
                  ...
                  autocomplete="off"
              ></el-input>
              <div class="login-code" @click="changeCode">
                <span class="login-code-img">{{code}}</span>
              </div>
            </div>
          </el-form-item>
			...
			this.$refs[formName].validate(valid => {
				if (valid) {
					...
					login(this.ruleForm).then(res=> {
							...
							let adminsRoles = ['admins'];
							let usersRoles = ['users'];
            let zhuguanRoles = ['zhuguan'];
							// admin 按钮权限标识
							let adminAuthBtnList = ['btn.add', 'btn.del', 'btn.edit', 'btn.link'];
							if (this.ruleForm.role === '管理员') {defaultRoles = adminsRoles;defaultAuthBtnList = adminAuthBtnList;}
							if (this.ruleForm.role === '用户') {defaultRoles = usersRoles;defaultAuthBtnList = adminAuthBtnList;}
            if (this.ruleForm.role === '老师') {
              defaultRoles = zhuguanRoles;
...
.role-radio-group ::v-deep .el-radio {
  margin-right: 0;
}

.role-radio-group ::v-deep .el-radio__label {
  color: #666;
  font-size: 14px;
}

.role-radio-group ::v-deep .el-radio__input.is-checked .el-radio__inner {
  background-color: #FF8D1B;
  border-color: #FF8D1B;
}

.role-radio-group ::v-deep .el-radio__input.is-checked + .el-radio__label {
  color: #FF8D1B;
}
